...
function LostAndFoundReport(props) {
  const locationId = props.match.params.locationId;
  const history = useHistory();
  const [lostAndFoundReports, setLossAndFoundReports] = useState([]);
  const [searchLostAndFoundReports, setSearchLostAndFoundReports] = useState(
    []
  );
  ...
  useEffect(() => {
    Axios.get(
      ...
      .then((response) => {
        ...
        setSearchLostAndFoundReports(response.data.data);
      })
      .catch((e) => {
  ...
  const searchReport = (e) => {
    ...
    searchLostAndFoundReports.map((report) => {
