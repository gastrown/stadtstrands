...
function AdminBrandPageManager () {
  ...
  const [modalFamilyAndFriendIcon, setModalFamilyAndFriendIcon] = useState(false);
  ...
  const toggleFreeIcon = () => { setModalFreeIcon(!modalFreeIcon); }

  const toggleDrinkIcon = () => { setModalDrinkIcon(!modalDrinkIcon); }

  const toggleSocialMediaIcon = () => { setModalSocialMediaIcon(!modalSocialMediaIcon); }

  const toggleLostAndFoundIcon = () => { setModalLostAndFoundIcon(!modalLostAndFoundIcon); }

  const toggleContactIcon = () => { setModalContactIcon(!modalContactIcon); }

  const toggleStrandorteIcon = () => { setModalStrandorteIcon(!modalStrandorteIcon); }

  const toggleAboutIcon = () => { setModalAboutIcon(!modalAboutIcon); }

  const toggleShopIcon = () => { setModalShopIcon(!modalShopIcon); }

  const toggleFamilyAndFriendIcon = () => { setModalFamilyAndFriendIcon(!modalFamilyAndFriendIcon); }

  const toggleTaxiIcon = () => { setModalTaxiIcon(!modalTaxiIcon); }

  const toggleEventIcon = () => { setModalEventIcon(!modalEventIcon); }

  const toggleFoodTruckIcon = () => { setModalFoodTruckIcon(!modalFoodTruckIcon); }

  const toggleReservationIcon = () => { setModalReservationIcon(!modalReservationIcon); }

  const toggleJobIcon = () => { setModalJobIcon(!modalJobIcon); } 

  const toggleDeliveryBoxIcon = () => { setModalDeliveryBoxIcon(!modalDeliveryBoxIcon); }
